% DELCODE-SAME, Figure "FADE SAME ApoE"
% _
...
% Figure 4: FADE and SAME scores by diagnostic group and ApoE genotype
% 
...
% finalized by Joram Soch <Joram.Soch@DZNE.de>, 10/10/2023, 16:49
...
% 
% % save results table
% filename = 'Figure_FADE_SAME_ApoE.xls';
% xlswrite(filename, tab);
% winopen(filename);


%%% Step 5b: visualize FADE-SAME scores %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% plot FADE/SAME scores
figure('Name', 'FADE/SAME, Fig. S3B', 'Color', [1 1 1], 'Position', [75 50 1600 900]);
...
for j = 1:numel(scores)
    ...
    text(mean([1 (2*num_grps)]), (-2+0.1), sprintf('ApoE main effect: %s', R2{2,j}), 'HorizontalAlignment', 'Center', 'VerticalAlignment', 'Middle');
    for k = 1:num_grps
    ...
    if j == 2
        legend(repmat({'E3 homozygotes', 'E4 carriers'}, [1 num_grps]), 'Location', 'SouthWest');
    end;
end;
