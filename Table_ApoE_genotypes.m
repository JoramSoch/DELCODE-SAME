% DELCODE-SAME, Table "ApoE genotypes"
% _
...
% finalized by Joram Soch <Joram.Soch@DZNE.de>, 10/10/2023, 16:47
...
%%% Step 4a: save chi^2 results %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% concatenate results table
frq = cell(1,numel(gens));
for k = 1:numel(gens), frq{k} = sprintf('%0.2f', prop(k)*100); end;
tab = [cell(1,1), gens, {'Statistics'}];
tab = [tab; {'population [%]'}, frq, {'Source: Li et al., BehGen, 2019'}];
for j = 1:num_grps
    for k = 1:numel(gens), frq{k} = sprintf('%0.2f', f(j,k)*100); end;
    tab = [tab; {sprintf('%s (N = %d)', subj_grps{j}, sum(g==j))}, frq, ...
                {sprintf('chi^2_%d = %2.2f, %s', df(j), chi2(j), pvalstr(pc(j), 0.001, []))}];
end;
clear frq

% save results table
filename = 'Table_ApoE_genotypes.xls';
